...
- **Estimate/quote request submission is resolved:** the owner's Jobber account handles this via its hosted public request form (`lib/constants.ts` → `JOBBER_REQUEST_URL`). `/get-estimate` and the "Request online" option on `/contact` link out to it; there is no local form submission, API route, or email-delivery service, and none is needed.
- **One dependency decision is still open and blocks real progress in Phase 12:** which hosting platform the site will deploy to. Not confirmed in any planning document — this should be resolved before that phase begins, not defaulted to arbitrarily during implementation.
